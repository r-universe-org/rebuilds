...
#' @rdname rebuilds
rebuild_oldies <- function(universe, before = Sys.Date()-32, types = c('src', 'failure'), delay = 0){
  subdomain <- paste(sprintf('%s.', universe), collapse = '')
...
rebuild_missing_winarm <- function(universe = 'cran'){
  ...
  winrepo <- as.data.frame(available.packages(ignore_repo_cache = TRUE, sprintf('https://%s.r-universe.dev/bin/windows/contrib/4.6/', universe)))
